...
from src.tools import set_iteration, save_model_scalar, save_node_scalar
...
class MyOptimizer(nn.Momentum):
    ...
    def construct(self, grads):
        for i in range(len(self.weight_names)):
            save_node_scalar(self.weight_names[i], self.parameters[i].data)
            save_node_scalar(self.gradient_names[i], grads[i])
        save_model_scalar("lr", self.get_lr())
...
class MyCallback(Callback):
    ...
    def step_begin(self, run_context):
        ...
        set_iteration(cb_params.cur_epoch_num, cb_params.cur_step_num)
...
model.train(cfg.epoch_size, ds_train, callbacks=[MyCallback()],

...
class MSGraph(Graph):
    ...
    def build_graph(self, graph_proto):
        ...
        print(
            "Build graph end, normal node count: %s, polymeric node " "count: %s.",
            len(self._normal_nodes),
            len(self._polymeric_nodes),
        )
    ...
    def _build_leaf_nodes(self, graph_proto):
        ...
        """
        print("Start to build leaf nodes.")
        leaf_node_id_map_name = {}
        const_nodes_map = {}

        for node_def in graph_proto.node:
            if not node_def.name:
                warnings.warn("Finding a node with an empty name will not save it.")
                continue
        ...
        for i, const in enumerate(graph_proto.const_vals):
            if not const.key:
                warnings.warn("Finding a const with an empty key will not save it.")
                continue
        ...
        for node_def in graph_proto.node:
            ...
            for input_def in node_def.input:
                ...
                if const_nodes_map.get(input_def.name):
                    ...
                    if src_node is None:
                        warnings.warn(
                            "The input '%s' in node '%s' is not in " "leaf nodes.",
                            src_name,
                            node_name,
                        )
                        continue
        ...
        for name, node in self._leaf_nodes.items():
            ...
            for src_name, input_attr in node.inputs.items():
                ...
                if self._normal_nodes.get(src_name):
                    ...
                    group_name = self._create_group_name(
                        src_node.name_scope, src_node.node_type, src_node.name
                    )
                    self._node_groups[group_name][src_name].update_output(
                        {name: input_attr}
                    )
    ...
    def _parse_graph_proto_node(self, node_def):
        ...
        """
        node_name = (
            "/".join([node_def.scope, node_def.op_type]) + node_def.name
            if node_def.scope
            else node_def.op_type + node_def.name
        )

...
class Graph:
    ...
    def search_single_node(self, node_name):
        ...
        if (
            node_name
            and self._polymeric_nodes.get(node_name) is None
            and self._normal_nodes.get(node_name) is None
        ):
            err_message = "Node not in graph."
            raise Exception(err_message)

        response = {}
        nodes = self.get_normal_nodes()
        response.update({"nodes": nodes, "scope_name": "", "children": {}})
        ...
        for i in range(1, len(names) + 1):
            if i == len(names):
                ...
                if polymeric_node:
                    ...
                    children.update(
                        {"nodes": nodes, "scope_name": polymeric_scope, "children": {}}
                    )
                break

            name_scope = "/".join(names[:i])
            nodes = self.get_normal_nodes(name_scope)
            children.update({"nodes": nodes, "scope_name": name_scope, "children": {}})
    ...
    def _build_polymeric_nodes(self):
        ...
        print("Start to build polymeric nodes")
        ...
        for name, node in self._polymeric_nodes.items():
            ...
            for src_name in node.inputs:
                ...
                polymeric_input.update(
                    {output_name: {"edge_type": EdgeTypeEnum.DATA.value}}
                )
            node.update_polymeric_input(polymeric_input)
        ...
        for node_name in getattr(node, attr):
            ...
            if node.node_type == NodeTypeEnum.POLYMERIC_SCOPE.value:
                node_name = (
                    ...
                    else polymeric_node.polymeric_scope_name
                )
                dummy_node_name = self._calc_dummy_node_name(node.name, node_name)
                polymeric_attr.update(
                    {dummy_node_name: {"edge_type": EdgeTypeEnum.DATA.value}}
                )
                continue
            ...
            if (
                node.name_scope == polymeric_node.name_scope
                or node.name_scope.startswith(polymeric_node.name_scope + "/")
            ):
                polymeric_attr.update(
                    {
                        polymeric_node.polymeric_scope_name: {

...
from .common.constants import (
    ...
    MAX_SCALAR_STEP_SIZE_PER_TAG,
    MAX_GRAPH_STEP_SIZE_PER_TAG,
)
...
_DEFAULT_STEP_SIZES_PER_TAG = DEFAULT_STEP_SIZES_PER_TAG

CONFIG = {
    "max_total_tag_sizes": MAX_TAG_SIZE_PER_EVENTS_DATA,
    "max_tag_sizes_per_plugin": {PluginNameEnum.GRAPH: MAX_GRAPH_TAG_SIZE,},
    "max_step_sizes_per_tag": {
        PluginNameEnum.SCALAR: MAX_SCALAR_STEP_SIZE_PER_TAG,
        PluginNameEnum.IMAGE: MAX_IMAGE_STEP_SIZE_PER_TAG,
        PluginNameEnum.GRAPH: MAX_GRAPH_STEP_SIZE_PER_TAG,
    },
}
...
class EventsData:
    ...
    def add_tensor_event(self, tensor_event):
        ...
        with self._reservoir_mutex_lock:
            if tag not in self._reservoir_by_tag:
                ...
                self._reservoir_by_tag[tag] = Reservoir(reservoir_size)
    ...
    def _check_tag_out_of_spec(self, plugin_name):
        ...
        if (
            tag_specifications is not None
            and len(self._tags_by_plugin[plugin_name]) >= tag_specifications
        ):

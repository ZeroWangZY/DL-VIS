...
const Snaphot: React.FC = () => {
  ...
  const brushed = () => {
    ...
    focusData.xScale.domain(
      s.map(contextData.xScale.invert, contextData.xScale)
    );
  ...
  const compute = async () => {
    ...
    contextData = computeXYScales([data], svgWidth, height2);
    focusData = computeXYScales([data], svgWidth, height);
    ...
    d3.select(svgRef.current)
      ...
      .on("onclick", function () {
        modifyGlobalConfigurations(
          GlobalConfigurationsModificationType.SET_CURRENT_SEPT,
          focusData.xScale.invert(clickNumber)
        );
      });
  };

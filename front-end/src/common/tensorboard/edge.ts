...
import { RenderMetaedgeInfo, RenderNodeInfo, RenderGraphInfo, Point } from "./render";
...
export type EdgeData = { v: string, w: string, label: RenderMetaedgeInfo };
...
export function buildGroup(sceneGroup,
  ...
  let container =
    selectOrCreateChild(sceneGroup, 'g', Class.Edge.CONTAINER);
...
function adjustPathPointsForMarker(points: Point[],
  marker: d3.Selection<any, any, any, any>, isStart: boolean): Point[] {
  let lineFunc = d3.line<Point>()
    .x(d => d.x)
    .y(d => d.y);
  let path =
...
export function appendEdge(edgeGroup, d: EdgeData,
  sceneElement: { renderHierarchy: RenderGraphInfo },
  edgeClass?: string) {
  let size = 1;
...
function getEdgePathInterpolator(d: EdgeData, i: number, a: string) {
  let renderMetaedgeInfo = <RenderMetaedgeInfo>d.label;

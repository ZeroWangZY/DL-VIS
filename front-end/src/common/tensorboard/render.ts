import {
  NodeType, createGraph, GraphType, GroupNode,
  Node, OpNode, getHierarchicalPath, Metanode, OpNodeImpl, Metaedge, createMetanode, MetaedgeImpl, FUNCTION_LIBRARY_NODE, BridgeNode, InclusionType, EllipsisNode, EllipsisNodeImpl
} from "./graph";
import * as d3 from "d3";
import { Hierarchy } from "./hierarchy";
import { EDGE_WIDTH_SCALE, MIN_EDGE_WIDTH, MAX_EDGE_WIDTH } from "./edge";
import * as graphlib from 'graphlib'
import { hasDisplayableNodeStats } from "./utils";
...
export class RenderGraphInfo {
  ...
  private cloneFunctionLibraryMetanode(
    ...
    const newMetanode = createMetanode(
  ...
  buildSubhierarchy(nodeName: string): void {
    ...
    if (!_.isEmpty(this.hierarchy.libraryFunctions)) {
      ...
      _.each(metagraph.nodes(), childName => {
        ...
        if (childName.indexOf(FUNCTION_LIBRARY_NODE) === 0) {
...
export const build = graphHierarchy => {
  if (graphHierarchy.root.type !== NodeType.META) {
    return;
  }
  let renderGraph = new RenderGraphInfo(graphHierarchy, !!null);
  ...
  //   computeTime: getColorParamsFromScale(renderGraph.computeTimeScale),
  ...
  //       };
  //     }),
  //   xlaCluster: _.map(renderGraph.xlaClusterColorMap.domain(),
  //     function (xlaClusterName) {
  //       return {
  //         xlaCluster: xlaClusterName,

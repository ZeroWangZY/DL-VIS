...
import { Node, NodeType, getIncludeNodeButtonString, OpNode, SeriesNode, SeriesGroupingType, getGroupSeriesNodeButtonString, Metanode, BridgeNode, GroupNode, OpNodeImpl, MetanodeImpl, getStrictName, ROOT_NAME } from './graph'
import * as annotation from './annotation'
import * as contextmenu from './contextmenu'
import { escapeQuerySelector } from './utils';
    ...
    case ColorBy.OP_COMPATIBILITY:
      if (renderInfo.node.type === NodeType.OP) {
        return ((<OpNode>renderInfo.node).compatible) ?
          render.OpNodeColors.COMPATIBLE :
          render.OpNodeColors.INCOMPATIBLE;
        ...
        if (linearGradient.size() === 0) {
          ...
          linearGradient.append('stop')
            .attr('offset', 0)
            .attr('stop-color', render.OpNodeColors.COMPATIBLE);
          linearGradient.append('stop')
            .attr('offset', percentValid)
            .attr('stop-color', render.OpNodeColors.COMPATIBLE);
          linearGradient.append('stop')
            .attr('offset', percentValid)
            .attr('stop-color', render.OpNodeColors.INCOMPATIBLE);
          linearGradient.append('stop')
            .attr('offset', 1)
            .attr('stop-color', render.OpNodeColors.INCOMPATIBLE);
        }

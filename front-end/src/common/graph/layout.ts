...
import { RenderGroupNodeInfo, RenderNodeInfo, RenderMetaedgeInfo, Point, Annotation, AnnotationType } from "./render";
 ...
 * Update layout, size, and annotations of its children nodes and edges.
 */
function layoutChildren(renderNodeInfo: RenderGroupNodeInfo): void {
...
function dagreLayout(
  ...
  _.each(graph.edges(), edgeObj => {
    ...
    _.each(edgeInfo.points, (point: Point) => {
  ...
  _.each(graph.edges(), edgeObj => {
    _.each(graph.edge(edgeObj).points, (point: Point) => {
      point.x -= minX;
      point.y -= minY;
    });
  });
...
function layoutMetanode(renderNodeInfo: RenderGroupNodeInfo): void {
 ...
 * series.
 */
function layoutSeriesNode(node: RenderGroupNodeInfo): void {
 ...
 *
 */
function layoutAnnotation(renderNodeInfo: RenderNodeInfo): void {
 ...
 */
function sizeAnnotation(a: Annotation): void {

...
function _buildEdgeMap(visNodes: NodeId[], visEdges: VisEdge[]): EdgeMap {
  const edgeMap = new Map<
    NodeId,
    { inEdges: Set<NodeId>; outEdges: Set<NodeId> }
  >();
  for (const nodeId of visNodes) {
    edgeMap.set(nodeId, {
      inEdges: new Set<NodeId>(),
      outEdges: new Set<NodeId>(),
    });
  }
...
function _findEndHubNode(
  ...
  for (const node of initialNodes) {
    ...
    while (traverseQueue.length > 0 && step < MAX_SUBGRAPH_DETECTING_STEP) {
      ...
      oldCount
        ? appearenceCountMap.set(nodeId, oldCount + 1)
        : appearenceCountMap.set(nodeId, 1);
...
function _findSubgraphsFromStartHubNode(
  startHubNode: NodeId,
  endHubNodes: Set<NodeId>,
  edgeMap: EdgeMap,
  nodeMap: VisNodeMap
): Set<Subgraph> {
...
function _findSubgraphsFromEndHubNode(
  endHubNode: NodeId,
  edgeMap: EdgeMap,
  nodeMap: VisNodeMap
): Set<Subgraph> {
...
function _getNodeHash(nodeId: NodeId, nodeMap: VisNodeMap): number {
  ...
  retHash =
    (retHash + genHash(node.outputNode.size.toString())) % BIG_PRIMITIVE;
  retHash = (retHash + genHash(node.auxiliary.size.toString())) % BIG_PRIMITIVE;
  return retHash;
}

function _getEdgeHash(
  ...
  let retHash =
    genHash(sourceNode.operationType + targetNode.operationType) %
    BIG_PRIMITIVE;
...
function _getSubgraphHash(subgraph: Subgraph, nodeMap): number {
  ...
  edges.forEach(
    (e) => (retHash = (retHash + _getEdgeHash(e, nodeMap)) % BIG_PRIMITIVE)
  );
  nodes.forEach(
    (n) => (retHash = (retHash + _getNodeHash(n, nodeMap)) % BIG_PRIMITIVE)
  );
  return retHash;
}

function _detectSimilarSubgraph(
  subgraphs: Set<Subgraph>,
  nodeMap: VisNodeMap
): Map<number, Set<Subgraph>> {
...
function _stackSimilarSubgraphs(
    ...
    subgraphToDelete.nodes.forEach((n) => {
      ...
      if (node instanceof StackedOpNodeImp) {
        (node as StackedOpNode).nodesContained.forEach((subnode) =>
          (nodeMap[
            hashToNodeMap.get(_getNodeHash(n, nodeMap))
          ] as StackedOpNode).nodesContained.add(subnode)
        );
      } else {
        (nodeMap[
          hashToNodeMap.get(_getNodeHash(n, nodeMap))
        ] as StackedOpNode).nodesContained.add(n);
      }
    });
  }
  ...
  for (let i = 0; i < visEdges.length; i++) {
    if (
      nodesToDeleteSet.has(visEdges[i].source) &&
      nodesToDeleteSet.has(visEdges[i].target)
    ) {
      const newSource = hashToNodeMap.get(
        _getNodeHash(visEdges[i].source, nodeMap)
      );
      const newTarget = hashToNodeMap.get(
        _getNodeHash(visEdges[i].target, nodeMap)
      );
      const oldEdge = visEdges.find(
      ...
      const oldEdge = visEdges.find(
        (e) => e.source === newSource && e.target === visEdges[i].target
      );
      if (!oldEdge) {
        visEdges[i].source = newSource;
      } else {
      ...
      const oldEdge = visEdges.find(
        (e) => e.source === visEdges[i].source && e.target === newTarget
      );
      if (!oldEdge) {
        visEdges[i].target = newTarget;
      } else {
...
function stackFrequentSubgraph(visGraph: VisGraph) {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    for (const startHubNode of startHubNodes) {
      ...
      if (
        edgeMap.get(startHubNode).outEdges.size >
        EDGE_COUNT_THRESHOLD_TO_TRIGER_STACK
      ) {
        ...
        const subgraphs = _findSubgraphsFromStartHubNode(
          startHubNode,
          endHubNodes,
          edgeMap,
          visNodeMap
        ...
        const similarSubgraphMap = _detectSimilarSubgraph(
          subgraphs,
          visNodeMap
        );
  ...
  for (let i = 0; i < 3; i++) {
    ...
    for (const endHubNode of endHubNodes) {
      ...
      const subgraphs = _findSubgraphsFromEndHubNode(
        endHubNode,
        edgeMap,
        visNodeMap
      );

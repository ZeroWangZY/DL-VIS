...
function parsePbtxtFile(
  input: ArrayBuffer,
  repeatedFields: { [attrPath: string]: boolean }): Promise<RawGraph> {
  let output: RawGraph = { node: undefined, versions: undefined };
  let stack: any[] = [];
  let path: string[] = [];
  let current: RawGraph = output;
...
const _fetchAndParseGraphData = (path: string, pbTxtFile: Blob | null): Promise<RawGraph> => {
  let result
  if (pbTxtFile) {
...
export const fetchAndParseGraphData: (path: string, pbTxtFile: Blob | null) => Promise<RawGraph>
  = wrapTaskWithTimeLogger(_fetchAndParseGraphData)
